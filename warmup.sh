...
function crawl() {
    ...
    LIST=$(curl -s -A "Mozilla/5.0 (https://github.com/will-parsons/scripts/blob/master/warmup.sh)" $1 | egrep -o 'href="[^"^\?]*"' | cut -d'"' -f2 | grep ^http://$BASE | egrep -v 'download|login|checkout|media|image|css|gallery|comments|blog|feed|account|uenc|review|sendfriend|wishlist|tag|price|filter|wp|\/l\/')

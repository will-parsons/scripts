...
function crawl() {
    ...
    LIST=$(curl -s $1 | egrep -o 'href="[^"^\?]*"' | cut -d'"' -f2 | grep ^http://$BASE | egrep -v 'media|comments|blog|feed|account|uenc|review-form|sendfriend|wishlist|tag|price|filter|wp|\/l\/')
